...
export default function HomePage({ onOpenPhotoSheet }: Props) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-120px)] flex-col">
      {/* ── Hero Section ── */}
      <div className="bg-card px-5 pt-8 pb-10">
        <div className="mx-auto w-full max-w-md">
          {/* Headline — 1초 안에 핵심 가치 전달 */}
          <h2 className="text-[26px] font-bold leading-[1.35] tracking-tight text-foreground">
            집에서 찍고,
            <br />
            <span className="text-primary">피부를 바꾸다</span>
          </h2>
          ...
          <button
            ...
            className="mt-7 flex w-full items-center justify-center gap-2 rounded-2xl h-[56px] text-[16px] font-semibold bg-primary text-white active:scale-[0.98] transition-all"
          >
            <Sparkles size={20} />
            피부 분석 시작하기
          </button>
      ...
      {/* ── Content Area ── */}
      <div className="flex-1 bg-background px-5 pt-6 pb-8">
        <div className="mx-auto w-full max-w-md space-y-5">
          ...
          {latestRecord && (
            <button
              ...
              className="flex w-full items-center gap-4 rounded-2xl bg-card p-5 text-left border border-border active:scale-[0.99] transition-all"
            >
              <SkinScoreCircle score={latestRecord.score} size="sm" />
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-medium text-muted-foreground">최근 피부 점수</p>
                <div className="mt-1 flex items-baseline gap-2">
                  <span className="text-2xl font-bold text-foreground">{Math.round(latestRecord.score)}</span>
          ...
          <div>
            <h3 className="mb-3 text-[13px] font-bold text-muted-foreground tracking-wide">분석 항목</h3>
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: Droplets, label: "피부 상태 분석", desc: "건조·색소·모공·주름·탄력", color: "#3182F6" },
                { icon: Search, label: "부위별 정밀 분석", desc: "이마·볼·눈가 등 9개 영역", color: "#3182F6" },
                ...
                >
                  ...
                  <p className="mt-3 text-[14px] font-semibold leading-[1.3] text-foreground">{item.label}</p>
                  <p className="mt-1 text-[12px] leading-[1.4] text-muted-foreground">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* How it Works — 간결한 스텝 */}
          <div>
            <h3 className="mb-3 text-[13px] font-bold text-muted-foreground tracking-wide">이용 방법</h3>
            <div className="flex items-start gap-4 rounded-2xl bg-card px-5 py-5 border border-border">
              {[
                { num: "1", label: "촬영" },
                { num: "2", label: "AI 분석" },
                { num: "3", label: "결과 확인" },
                ...
                <div key={item.num} className="flex flex-1 flex-col items-center text-center">
                  ...
                  <p className="mt-2 text-[13px] font-medium text-foreground">{item.label}</p>
                </div>
              ))}
            </div>
          </div>
